// Headless verification for the quick-chat panel: boot the web profile like
// src/main.js and assert (1) the plugin tree loads, (2) dsh-client-ui-quickchat
// is in the client-modules graph, and (3) the removed pet bundle is gone.
...
try {
  ...
  const qc = entries.find((r) => r.id === "@deepseek-ai/dsh-client-ui-quickchat");
  if (qc === undefined) throw new Error("ui-quickchat missing from client-modules graph");
  if (clientModules.clientPath(qc.id) === undefined) throw new Error(`ui-quickchat clientPath missing (${qc.url})`);
  console.log(`bundle OK: ui-quickchat -> ${qc.url}`);

  if (entries.some((r) => r.id === "@deepseek-ai/dsh-client-ui-pet")) {
    throw new Error("pet bundle still present in client-modules graph");
  }
  console.log("bundle OK: ui-pet removed");

  console.log("quickchat verify OK");

...
function spawnDialogWorker(data) {
	const env = {
		...
		// is the app binary: without this switch the worker boots the full app,
		// loses the single-instance lock, and quits before reporting a result.
		// Electron only reads the variable for ITS OWN startup, so scoping it to
		// this spawn is safe (a process-wide export would break the renderer and
		// GPU children instead).

...
/**
* The request modalities a listing entry declares, or `undefined` when absent.
* OpenAI-compatible endpoints disclose them as `input_modalities` (some also as
* `modalities`); only the two modalities this build models are kept, so a value
* no schema can carry is dropped rather than persisted.
*/
function modalities(...candidates) {
	const declared = new Set();
	for (const candidate of candidates) {
		if (!Array.isArray(candidate)) continue;
		for (const value of candidate) if (value === "text" || value === "image") declared.add(value);
	}
...
function readListing(body) {
	...
	for (const raw of data) {
		...
		const inputModalities = modalities(entry?.input_modalities, entry?.modalities);
		models.push({
			id,
			...name === void 0 ? {} : { name },
